        ...
        <div className="ml-3 w-full">
          ...
          <div className="mt-3 space-y-4">
            {trades.map(trade => (
              <div 
                key={trade.id} 
                className="bg-white dark:bg-neutral-800 rounded-lg shadow p-4 border border-yellow-200 dark:border-yellow-900"
              >
                        ...
                        <div className="bg-white dark:bg-neutral-800 rounded-lg p-3 shadow-sm">
                          <ul className="space-y-2">
                            {(trade.players_received_details || []).map(player => (
                              <li key={player.id} className="text-sm text-neutral-800 dark:text-neutral-200">
                                {player.name}
                              </li>
                            ))}
                            {(trade.players_received_details || []).length === 0 && (
                              <li className="text-sm text-neutral-500 dark:text-neutral-400 italic">
                                No players
                              </li>
                            )}
                          </ul>
