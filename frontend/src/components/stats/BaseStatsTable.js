          ...
          <tr>
            {visibleColumns.map((column) => (
              <th 
                key={column.key}
                scope="col" 
                className={`px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider ${column.sortable !== false ? 'cursor-pointer' : ''}`}
              ...
              {visibleColumns.map((column) => {
                ...
                return (
                  <td 
                    ...
                    className="px-3 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-800 dark:text-gray-200"
                  >
                    {renderedValue}
                  </td>
                );
